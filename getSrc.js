  ...
  try {
    ...
    for (let i = 0; i < albumLinks.length; i++) {
      ...
      try {
        ...
        let numOfTracks = await page.$$eval(
          '.track-single',
    ...
    // close the browser
    await browser.close();
    // Call appendFile Here and pass true as an argument to state that the data has been scraped, end the json file
  } catch (error) {
    console.log(error);
  }
}
